...
export default function PokemonSilhouette() {
  ...
  const [player1AnsweredCorrectly, setPlayer1AnsweredCorrectly] = useState(false);
  const [player2AnsweredCorrectly, setPlayer2AnsweredCorrectly] = useState(false);
    ...
    <div className="container">
      ...
      <div className="board-container">
        ...
        {currentPlayer === 1 ? (
          ...
          <input style={{color:"green"}} value={player2InputValue} type='text' onChange={(event) => setPlayer2InputValue(event.target.value)} onKeyDown={registerPlayer2Answer}></input>
        )}
      </div>
    </div>
  );
